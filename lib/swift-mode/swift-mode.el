...
(defcustom swift-offset-c 2
  "Default indentation width for the body of multiline comments."
  :type 'integer)
...
(defun swift-indent-line ()
  ...
  (let (indent-level target-column)
    (save-excursion
      ...
      (let ((synp (syntax-ppss (point-at-bol))))
        (setq indent-level (car synp))
        (beginning-of-line)
        (skip-syntax-forward " ")
        (setq
         target-column
         (cond ((or (equal (char-after) ?\#) (looking-at "//:")) 0)
               ((and (nth 4 synp)
                     (not (looking-at ".*/[*/]\\|.*\\*/")))
                (+ (* swift-basic-offset indent-level)
                   swift-offset-c))
               (t (* swift-basic-offset
                     (- indent-level
